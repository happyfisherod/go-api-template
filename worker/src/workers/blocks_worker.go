...
func blocksWorker() {
	...
	if utils.StringInSlice(consumer_topic_name, strings.Split(config.Vars.ConsumerTopics, ",")) == false {
		log.Panic("Worker blocks: invalid BLOCKS_WORKER_CONSUMER_TOPIC value. MUST be a topic in CONSUMER_TOPICS")
	}
	if utils.StringInSlice(producer_topic_name, strings.Split(config.Vars.ProducerTopics, ",")) == false {
		log.Panic("Worker blocks: invalid BLOCKS_WORKER_PRODUCER_TOPIC value. MUST be a topic in PRODUCER_TOPICS")
	}
	...
	log.Debug("Worker blocks: started working")
	for {
		// Consume
		consumer_topic_msg := <-consumer_topic_chan
		log.Debug("Worker blocks: last consumed block #", string(consumer_topic_msg.Key))
